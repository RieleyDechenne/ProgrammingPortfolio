...
to setup
  ...
  ask patches with [(random-float 100) < density]
    [ set pcolor green ]
                                                                                 ;;Code added for the first extension ↓
  ask patches with [pcolor = red] [
    ask neighbors4 with [pcolor = green] [                                       ;;Code allows each red patch to search it's four neighbors (N, S, E, W)
      set pcolor red]                                                            ;;And sets the ones that are green to red simulating sparks flying and creating more fire.
    ]                                                                            ;;Code added for the first extension ↑
...
to go
  if not any? turtles  ;; either fires or embers
    [ stop ]
  ask fires [
                                                                                 ;;Added this code for the third extension ↓
    let fire-range neighbors4                                                    ;;This added code is dealing with the switch button.
    if Big-Jump? [ set fire-range neighbors ]                                    ;;Added this code for the third extension ↑


    ask fire-range with [pcolor = green] [

      let probability (Probability-of-spread / 100)                              ;;Code added for the second extension ↓

      if [pycor] of myself < pycor [                                             ;;This is the code for the south-wind-spread
        set probability probability + (south-wind-spread / 100)]                 ;;Code added for the second extension

      if [pxcor] of myself < pxcor [                                             ;;Code added for the second extension.
        set probability probability + (West-wind-spread / 100)]                  ;;This is the code for the west-wind-spread
                                                                                 ;;Code added for the second extension ↑
      if random-float 1 < probability [
        ignite ]
    ]

    if Big-Jump? [                                                               ;;Code added for the third extensions ↓
      if random-float 1 < (Probability-of-spread / 100) [                        ;;Uses the slider to determine the chance of a big jump.
      set pcolor red                                                             ;;Marks the current patch red, showing that it is actively burning.

      let jump-dx 3
      let jump-dy 3                                                              ;;Used a fixed number of jump offsets (3 patches E, 3 patches N)
      let target patch-at jump-dx jump-dy                                        ;;Finds a patch at a relative position determined by jump-dx and jump-dy.

      if target != nobody and [pcolor] of target = green [                       ;;Ignites the target patch if it's unburned.
        ask target [ ignite ]                                                    ;;Code added for the third extensions ↑
      ;ask patches with [pcolor = green and distance myself < 3] [
        ;if random-float 1 < (Probability-of-spread / 2) [
          ;ignite
        ]
      ]
      set breed embers
